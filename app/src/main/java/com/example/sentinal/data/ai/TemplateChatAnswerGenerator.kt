     ...
     override suspend fun generateAnswer(
        ...
        return when (intent) {
            ChatIntent.DeviceStatus -> {
                if (guardian == null) {
                    ChatAnswer(
                        text = "아직 최근 기기 상태 데이터가 충분하지 않습니다. 사용정보 접근 권한과 데이터 수집 상태를 확인해 주세요.",
                        source = AiResponseLabels.GUARDIAN_RECENT_DATA,
            ...
            ChatIntent.BatteryConcern -> {
                ChatAnswer(
                    ...
                    source = AiResponseLabels.ANALYTICS_WEEKLY_USAGE_DATA,
                    modelTier = ModelTier.Template
                )
            }
            ...
            ChatIntent.SecurityCheck -> {
                if (guardian == null) {
                    ChatAnswer(
                        text = "이 앱은 백신처럼 바이러스를 검사하지 않습니다. 또한 아직 최근 리소스 데이터가 부족해 이상 징후 참고도 어렵습니다. 정확한 검사는 안드로이드 보안 설정 또는 신뢰할 수 있는 보안 앱을 사용해 주세요.",
                        source = AiResponseLabels.TEMPLATE_SAFETY_GUIDE,
                        modelTier = ModelTier.Template
                    )
                } else {
                    ChatAnswer(
            ...
            ChatIntent.CategoryUsage -> {
                ...
                if (topCategory == null) {
                    ChatAnswer(
                        ...
                        source = AiResponseLabels.ANALYTICS_CATEGORY_DATA,
                        modelTier = ModelTier.Template
                    )
                } else {
                    ChatAnswer(
            ...
            ChatIntent.MostUsedApp -> {
                ...
                if (topApp == null) {
                    ChatAnswer(
                        ...
                        source = AiResponseLabels.ANALYTICS_APP_USAGE_DATA,
                        modelTier = ModelTier.Template
                    )
                } else {
                    ChatAnswer(
            ...
            ChatIntent.NightUsage -> {
                ChatAnswer(
                    ...
                    source = AiResponseLabels.ANALYTICS_NIGHT_USAGE_DATA,
                    modelTier = ModelTier.Template
                )
            }
            ...
            ChatIntent.Unsupported -> {
                ChatAnswer(
                    ...
                    source = AiResponseLabels.TEMPLATE_FALLBACK,
                    modelTier = ModelTier.Template
                )
            }
        }
